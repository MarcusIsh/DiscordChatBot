                    ...
                    if(data.id == null){
                        ...
                        request(playerRankData, function (error, response, body) {
                            ...
                            db.query("select summonerName from users where summonerName ='" + rankData[0].summonerName + "'", function (err, result) {
                                if(err) throw err;
                                var insVar = "insert into users (summonerName, summonerLevel, summonerId, tier, rank, playerId) values ( '"+ rankData[0].summonerName +"','"+ data[0].summonerLevel +"','"+ rankData[0].summonerId+"','"+ rankData[0].tier +"','"+ rankData[0].rank +"','"+ rankData[0].leagueId +"')";
                                console.log(insVar);
                                if(!result) {
